			       ...
			       unsigned long bitmap_nr)
{
	...
	if (!bh)
		bext2_panic (sb, "read_block_bitmap",
			       ...
			       unsigned int block_group)
{
	...
	if (block_group >= sb->u.ext2_sb.s_groups_count)
		bext2_panic (sb, "load_block_bitmap",
			    "block_group >= groups_count - "
			    "block_group = %d, groups_count = %lu",
			    block_group, sb->u.ext2_sb.s_groups_count);
...
void bext2_free_blocks (const struct inode * inode, unsigned long block,
		       unsigned long count)
{
	struct buffer_head * bh;
	...
	sb = inode->i_sb;
	if (!sb) {
		printk ("bext2_free_blocks: nonexistent device");
		return;
	}
	lock_super (sb);
	...
	if (block < es->s_first_data_block || 
		...
		bext2_error (sb, "bext2_free_blocks",
	...
	for (i = 0; i < count; i++) {
		if (!clear_bit (bit + i, bh->b_data))
			bext2_warning (sb, "bext2_free_blocks",
...
/*
 * bext2_new_block uses a goal block to assist allocation.  If the goal is
 * free, or there is a free block within 32 blocks of the goal, that block
 * is allocated.  Otherwise a forward search is made for a free block; within 
 * each block group the search first looks for an entire free byte in the block
 * bitmap, and then for any free bit if that fails.
 */
int bext2_new_block (const struct inode * inode, unsigned long goal,
		    u32 * prealloc_count, u32 * prealloc_block, int * err)
{
	...
	*err = -ENOSPC;
	...
	sb = inode->i_sb;
	if (!sb) {
		printk ("bext2_new_block: nonexistent device");
		return 0;
	}
	lock_super (sb);
	es = sb->u.ext2_sb.s_es;
...
repeat:
	...
	if (gdp->bg_free_blocks_count > 0) {
		...
		bext2_debug ("goal is at %d:%d.\n", i, j);

		if (!test_bit(j, bh->b_data)) {
#ifdef EXT2FS_DEBUG
			goal_hits++;
			bext2_debug ("goal bit allocated.\n");
#endif
			goto got_block;
		}
		if (j) {
		...
		bext2_debug ("Bit not found near goal\n");
...
#if defined(__powerpc__) || defined(hp_pa)
		k ^= (3<<3);  /* Big/Little endian bit mapping problems! */
#endif
	...
	bext2_debug ("Bit not found in block group %d.\n", i);
	...
	j = (r - bh->b_data) << 3;
#if defined(__powerpc__) || defined(hp_pa)
	j ^= (3<<3);  /* Big/Little endian bit mapping problems! */
#endif		
	if (j < EXT2_BLOCKS_PER_GROUP(sb))
		...
		j = find_first_zero_bit ((unsigned long *) bh->b_data,
					 EXT2_BLOCKS_PER_GROUP(sb));
	if (j >= EXT2_BLOCKS_PER_GROUP(sb)) {
		bext2_error (sb, "bext2_new_block",
...
got_block:
	...
	bext2_debug ("using block group %d(%d)\n", i, gdp->bg_free_blocks_count);
	...
	if (test_opt (sb, CHECK_STRICT) &&
		...
		bext2_panic (sb, "ext2_new_block",
			    "Allocating block in system zone - "
			    "block = %u", tmp);

	if (set_bit (j, bh->b_data)) {
		bext2_warning (sb, "bext2_new_block",
	...
	if (prealloc_block) {
		...
		*prealloc_block = tmp + 1;
		...
		bext2_debug ("Preallocated a further %lu bits.\n",
			    *prealloc_count);
	}
#endif
	...
	if (!(bh = getblk (sb->s_dev, j, sb->s_blocksize))) {
		bext2_error (sb, "bext2_new_block", "cannot get block %d", j);
		unlock_super (sb);
		return 0;
	}
	...
	bext2_debug ("allocating block %d. "
...
unsigned long bext2_count_free_blocks (struct super_block * sb)
{
#ifdef EXT2FS_DEBUG
	struct ext2_super_block * es;
	...
	for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
		...
		bitmap_nr = load_block_bitmap (sb, i);
		x = bext2_count_free (sb->u.ext2_sb.s_block_bitmap[bitmap_nr],
				     sb->s_blocksize);
		printk ("group %d: stored = %d, counted = %lu\n",
			i, gdp->bg_free_blocks_count, x);
		bitmap_count += x;
	}
	printk("bext2_count_free_blocks: stored = %lu, computed = %lu, %lu\n",
...
void bext2_check_blocks_bitmap (struct super_block * sb)
{
	struct buffer_head * bh;
	struct ext2_super_block * es;
	unsigned long desc_count, bitmap_count, x;
	...
	for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
		...
		if (!test_bit (0, bh->b_data))
			bext2_error (sb, "bext2_check_blocks_bitmap",
		...
		x = bext2_count_free (bh, sb->s_blocksize);
