...
static int bext2_update_inode(struct inode * inode, int do_sync);

void bext2_put_inode (struct inode * inode)
{
	...
	inode->i_dirt = 1;
	bext2_update_inode(inode, IS_SYNC(inode));
	inode->i_size = 0;
	if (inode->i_blocks)
		bext2_truncate (inode);
	bext2_free_inode (inode);
}
...
/* 
 * bext2_discard_prealloc and bext2_alloc_block are atomic wrt. the
 * superblock in the same manner as are bext2_free_blocks and
 * bext2_new_block.  We just wait on the super rather than locking it
 * here, since bext2_new_block will do the necessary locking and we
 * can't block until then.
 */
void bext2_discard_prealloc (struct inode * inode)
{
	...
	if (inode->u.ext2_i.i_prealloc_count) {
		...
		bext2_free_blocks (inode, inode->u.ext2_i.i_prealloc_block, total);
	}
#endif
}

static int bext2_alloc_block (struct inode * inode, unsigned long goal, int * err)
{
#ifdef EXT2FS_DEBUG
	...
	{		
		...
		bext2_debug ("preallocation hit (%lu/%lu).\n",
		...
		if (!(bh = getblk (inode->i_sb->s_dev, result,
			...
			bext2_error (inode->i_sb, "bext2_alloc_block",
		...
		bext2_discard_prealloc (inode);
		bext2_debug ("preallocation miss (%lu/%lu).\n",
			    alloc_hits, ++alloc_attempts);
		if (S_ISREG(inode->i_mode))
			result = bext2_new_block (inode, goal,
				 &inode->u.ext2_i.i_prealloc_count,
				 &inode->u.ext2_i.i_prealloc_block, err);
		else
			result = bext2_new_block (inode, goal, 0, 0, err);
	}
#else
	result = bext2_new_block (inode, goal, 0, 0, err);
#endif
...
int bext2_bmap (struct inode * inode, int block)
{
	int i;
	...
	if (block < 0) {
		bext2_warning (inode->i_sb, "bext2_bmap", "block < 0");
		return 0;
	}
	if (block >= EXT2_NDIR_BLOCKS + addr_per_block +
		(1 << (addr_per_block_bits * 2)) +
		((1 << (addr_per_block_bits * 2)) << addr_per_block_bits)) {
		bext2_warning (inode->i_sb, "bext2_bmap", "block > big");
		return 0;
	}
...
repeat:
	...
	tmp = bext2_alloc_block (inode, goal, err);
	if (!tmp)
		return NULL;
	result = getblk (inode->i_dev, tmp, inode->i_sb->s_blocksize);
	if (*p) {
...
struct buffer_head * bext2_getblk (struct inode * inode, long block,
				  int create, int * err)
{
	struct buffer_head * bh;
	unsigned long b;
	...
	*err = -EIO;
	if (block < 0) {
		bext2_warning (inode->i_sb, "bext2_getblk", "block < 0");
		return NULL;
	}
	if (block > EXT2_NDIR_BLOCKS + addr_per_block +
		(1 << (addr_per_block_bits * 2)) +
		((1 << (addr_per_block_bits * 2)) << addr_per_block_bits)) {
		bext2_warning (inode->i_sb, "bext2_getblk", "block > big");
		return NULL;
	}
	/*
	...
	bext2_debug ("block %lu, next %lu, goal %lu.\n", block, 
...
struct buffer_head * bext2_bread (struct inode * inode, int block, 
				 int create, int *err)
{
	struct buffer_head * bh;

	bh = bext2_getblk (inode, block, create, err);
...
void bext2_read_inode (struct inode * inode)
{
	struct buffer_head * bh;
	...
	if ((inode->i_ino != EXT2_ROOT_INO && inode->i_ino != EXT2_ACL_IDX_INO &&
		...
		bext2_error (inode->i_sb, "bext2_read_inode",
			    "bad inode number: %lu", inode->i_ino);
		return;
	}
	block_group = (inode->i_ino - 1) / EXT2_INODES_PER_GROUP(inode->i_sb);
	if (block_group >= inode->i_sb->u.ext2_sb.s_groups_count)
		bext2_panic (inode->i_sb, "bext2_read_inode",
			    "group >= groups count");
	...
	if (inode->i_ino == EXT2_ACL_IDX_INO ||
		...
		inode->i_op = &bext2_file_inode_operations;
	else if (S_ISDIR(inode->i_mode))
		inode->i_op = &bext2_dir_inode_operations;
	else if (S_ISLNK(inode->i_mode))
		inode->i_op = &bext2_symlink_inode_operations;
...
static int bext2_update_inode(struct inode * inode, int do_sync)
{
	...
	if ((inode->i_ino != EXT2_ROOT_INO &&
	     inode->i_ino < EXT2_FIRST_INO(inode->i_sb)) ||
	    inode->i_ino > inode->i_sb->u.ext2_sb.s_es->s_inodes_count) {
		bext2_error (inode->i_sb, "bext2_write_inode",
			    "bad inode number: %lu", inode->i_ino);
		return 0;
	}
	block_group = (inode->i_ino - 1) / EXT2_INODES_PER_GROUP(inode->i_sb);
	if (block_group >= inode->i_sb->u.ext2_sb.s_groups_count)
		bext2_panic (inode->i_sb, "bext2_write_inode",
			    "group >= groups count");
	group_desc = block_group >> EXT2_DESC_PER_BLOCK_BITS(inode->i_sb);
	desc = block_group & (EXT2_DESC_PER_BLOCK(inode->i_sb) - 1);
	bh = inode->i_sb->u.ext2_sb.s_group_desc[group_desc];
	...
	if (do_sync) {
		...
		if (buffer_req(bh) && !buffer_uptodate(bh)) {
			printk ("IO error syncing bext2 inode ["
				"%s:%08lx]\n",
...
void bext2_write_inode (struct inode * inode)
{
	bext2_update_inode (inode, 0);
}

int bext2_sync_inode (struct inode *inode)
{
	return bext2_update_inode (inode, 1);
}
