    ...
    u32 *issue)
{
     ...
     */
	(struct NCR53c7x0_cmd *) ((char *) bus_to_virt (_LE_to_BE_long(issue[1])) - 
...
NCR53c7x0_driver_init (struct Scsi_Host *host) {
	...
	current[1] = _LE_to_BE_long(0xdeadbeef);
    }
    current[0] = _LE_to_BE_long(((DCMD_TYPE_TCI|DCMD_TCI_OP_JUMP) << 24) | DBC_TCI_TRUE);
    current[1] = (u32) _LE_to_BE_long(virt_to_bus (hostdata->script) +
	hostdata->E_wait_reselect);
    hostdata->reconnect_dsa_head = 0;
    hostdata->addr_reconnect_dsa_head = (u32) 
	_LE_to_BE_long(virt_to_bus((void *) &(hostdata->reconnect_dsa_head)));
    hostdata->expecting_iid = 0;
    hostdata->expecting_sto = 0;
    if (hostdata->options & OPTION_ALWAYS_SYNCHRONOUS) 
	hostdata->initiate_sdtr = _LE_to_BE_long(0xffff); 
    else
...
/* Is the SCSI clock ever anything else on these chips? */
    ...
    hostdata->NCR53c7xx_msg_reject = _LE_to_BE_long(MESSAGE_REJECT);
    hostdata->NCR53c7xx_msg_abort = _LE_to_BE_long(ABORT);
    hostdata->NCR53c7xx_msg_nop = _LE_to_BE_long(NOP);
    hostdata->NOP_insn = _LE_to_BE_long((DCMD_TYPE_TCI|DCMD_TCI_OP_JUMP) << 24);
...
/* The PowerPC is Big Endian - adjust script appropriately */
    ...
    {
        *script_ptr++ = _LE_to_BE_long(*script_ptr);
    }
#endif    	    
...
/* Need to adjust for endian-ness */    	
    	dsa[2] = _LE_to_BE_long(1);
    	dsa[3] = _LE_to_BE_long(virt_to_bus(&identify));
    	dsa[4] = _LE_to_BE_long(6);
    	dsa[5] = _LE_to_BE_long(virt_to_bus(&cmd));
    	dsa[6] = _LE_to_BE_long(sizeof(data));
    	dsa[7] = _LE_to_BE_long(virt_to_bus(&data));
    	dsa[8] = _LE_to_BE_long(1);
    	dsa[9] = _LE_to_BE_long(virt_to_bus(&status));
    	dsa[10] = _LE_to_BE_long(1);
    	dsa[11] = _LE_to_BE_long(virt_to_bus(&msg));
	...
	for (i = 0; i < 3; ++i) {
	    ...
	    /*	     SCNTL3         SDID	*/
	    dsa[0] = _LE_to_BE_long((0x33 << 24) | (i << 16))  ;
	    hostdata->idle = 0;
...
NCR53c8xx_dsa_fixup (struct NCR53c7x0_cmd *cmd) {
    ...
    {
       *dsa_ptr++ = _LE_to_BE_long(*dsa_ptr);
    }
#endif    	
...
abnormal_finished (struct NCR53c7x0_cmd *cmd, int result) {
    ...
    for (left = host->can_queue,
	    ncr_search = _LE_to_BE_long(hostdata->reconnect_dsa_head), 
	    ncr_prev = &hostdata->reconnect_dsa_head;
	left >= 0 && ncr_search && 
	    ((char*)bus_to_virt(ncr_search) + hostdata->dsa_start) 
		!= (char *) cmd->dsa;
	ncr_prev = (u32*) ((char*)bus_to_virt(ncr_search) + 
	    hostdata->dsa_next), ncr_search = _LE_to_BE_long(*ncr_prev), --left);
 ...
 */
	 ...
	 *		period >= requested period		 	
   	 */
		...
		patch_dsa_32 (cmd->dsa, dsa_msgout_other, 0, _LE_to_BE_long(5));
		patch_dsa_32 (cmd->dsa, dsa_msgout_other, 1, (u32) 
		    _LE_to_BE_long(virt_to_bus ((void *)&hostdata->msg_buf)));
 ...
 */
	 ...
	 */
    	...
    	patch_dsa_32 (cmd->dsa, dsa_msgout, 0, _LE_to_BE_long(1));
    	...
    	patch_dsa_32 (cmd->dsa, dsa_cmdout, 0, _LE_to_BE_long(6));
	 ...
	 */
	    ...
	    _LE_to_BE_long(virt_to_bus(hostdata->script) + hostdata->E_other_transfer));
    	patch_dsa_32 (cmd->dsa, dsa_datain, 0, 
	    _LE_to_BE_long(virt_to_bus(cmd->data_transfer_start)));
    	cmd->data_transfer_start[0] = _LE_to_BE_long((((DCMD_TYPE_BMI | DCMD_BMI_OP_MOVE_I | 
    	    DCMD_BMI_IO)) << 24) | sizeof(c->sense_buffer));
    	cmd->data_transfer_start[1] = (u32) _LE_to_BE_long(virt_to_bus(c->sense_buffer));

	cmd->data_transfer_start[2] = _LE_to_BE_long(((DCMD_TYPE_TCI | DCMD_TCI_OP_JUMP) 
    	    << 24) | DBC_TCI_TRUE);
	cmd->data_transfer_start[3] = (u32) _LE_to_BE_long(virt_to_bus(hostdata->script) + 
	...
	cmd->cmd->result = _LE_to_BE_long(0xffff);		
	...
	if (hostdata->options & (OPTION_DEBUG_SCRIPT|OPTION_DEBUG_INTR|
	    ...
	    printk("scsi%d : resume address is 0x%x (virt 0x%p)\n",
		    ...
		    bus_to_virt(_LE_to_BE_long(cmd->saved_data_pointer)));
    ...
    case A_int_debug_restored:
		...
		int size;
		    ...
		    _LE_to_BE_long(cmd->saved_data_pointer)));
		size = print_insn (host, (u32 *) 
		    bus_to_virt(_LE_to_BE_long(cmd->saved_data_pointer)), "", 1);
		size = print_insn (host, (u32 *) 
		    bus_to_virt(_LE_to_BE_long(cmd->saved_data_pointer)) + size, "", 1);
    	    	print_progress (c);
	    }
#if 0
	...
	case A_int_debug_datain:
		...
		size = print_insn (host, (u32 *) bus_to_virt(_LE_to_BE_long(dsp[1])), "", 1);
		print_insn (host, (u32 *) bus_to_virt(_LE_to_BE_long(dsp[1])) + size, "", 1);
	   } 
	return SPECIFIC_INT_RESTART;
#endif
/* 
...
#ifdef __powerpc__
{
    ...
    if (isBeBox[0])
    {
       NCR53c7x0_write8(STEST1_REG_800, STEST1_800_SCLK);
    }
}
#endif
...
create_cmd (Scsi_Cmnd *cmd) {
	...
	*cmd_dataout;		/* Incremented as we assemble */
    ...
    tmp->saved_data_pointer = _LE_to_BE_long(virt_to_bus (hostdata->script) + 
    ...
    tmp->next = NULL;
    tmp->flags = 0;
    tmp->dsa_next_addr = _LE_to_BE_long(virt_to_bus(tmp->dsa) + hostdata->dsa_next - 
	hostdata->dsa_start);
    tmp->dsa_addr = _LE_to_BE_long(virt_to_bus(tmp->dsa) - hostdata->dsa_start);
    ...
    patch_dsa_32(tmp->dsa, dsa_next, 0, _LE_to_BE_long(0));
    patch_dsa_32(tmp->dsa, dsa_cmnd, 0, _LE_to_BE_long(virt_to_bus(cmd)));
    ...
    patch_dsa_32(tmp->dsa, dsa_select, 0, _LE_to_BE_long(hostdata->sync[cmd->target].
    	select_indirect));
    /*
	...
	memcpy ((void *) (tmp->select + 1), (void *) wdtr_message,
	    sizeof(wdtr_message));
    	patch_dsa_32(tmp->dsa, dsa_msgout, 0, _LE_to_BE_long(1 + sizeof(wdtr_message)));
	save_flags(flags);
	cli();
	...
	memcpy ((void *) (tmp->select + 1), (void *) sdtr_message, 
	    sizeof(sdtr_message));
    	patch_dsa_32(tmp->dsa, dsa_msgout, 0, _LE_to_BE_long(1 + sizeof(sdtr_message)));
	...
	memcpy ((void *) (tmp->select + 1), (void *) async_message, 
    	...
    	patch_dsa_32(tmp->dsa, dsa_msgout, 0, _LE_to_BE_long(1 + sizeof(async_message)));
	tmp->flags |= CMD_FLAG_SDTR;
    } 
#endif
    else 
    	patch_dsa_32(tmp->dsa, dsa_msgout, 0, _LE_to_BE_long(1));
    hostdata->talked_to |= (1 << cmd->target);
    tmp->select[0] = (hostdata->options & OPTION_DISCONNECT) ? 
	IDENTIFY (1, cmd->lun) : IDENTIFY (0, cmd->lun);
    patch_dsa_32(tmp->dsa, dsa_msgout, 1, _LE_to_BE_long(virt_to_bus(tmp->select)));
    patch_dsa_32(tmp->dsa, dsa_cmdout, 0, _LE_to_BE_long(cmd->cmd_len));
    patch_dsa_32(tmp->dsa, dsa_cmdout, 1, _LE_to_BE_long(virt_to_bus(cmd->cmnd)));
    patch_dsa_32(tmp->dsa, dsa_dataout, 0, _LE_to_BE_long(cmd_dataout ? 
    	    virt_to_bus (cmd_dataout)
	: virt_to_bus (hostdata->script) + hostdata->E_other_transfer));
    patch_dsa_32(tmp->dsa, dsa_datain, 0, _LE_to_BE_long(cmd_datain ? 
     ...
     */
    patch_dsa_32(tmp->dsa, dsa_msgin, 0, _LE_to_BE_long(1));
/* 
 ...
 */
    patch_dsa_32(tmp->dsa, dsa_msgin, 1, _LE_to_BE_long(virt_to_bus(&cmd->result) + 1));
    patch_dsa_32(tmp->dsa, dsa_status, 0, _LE_to_BE_long(1));
    patch_dsa_32(tmp->dsa, dsa_status, 1, _LE_to_BE_long(virt_to_bus(&cmd->result)));
    patch_dsa_32(tmp->dsa, dsa_msgout_other, 0, _LE_to_BE_long(1));
    patch_dsa_32(tmp->dsa, dsa_msgout_other, 1, 
	_LE_to_BE_long(virt_to_bus(&(hostdata->NCR53c7xx_msg_nop))));
 ...
 */
	...
	if (datain) {
	    ...
	    cmd_datain[0] = _LE_to_BE_long(((DCMD_TYPE_TCI | DCMD_TCI_OP_CALL | 
		DCMD_TCI_IO) << 24) | 
		DBC_TCI_WAIT_FOR_VALID | DBC_TCI_COMPARE_PHASE);
	    cmd_datain[1] = _LE_to_BE_long(virt_to_bus (hostdata->script) + 
		hostdata->E_other_in);
	    /* MOVE count, buf, WHEN DATA_IN */
	    cmd_datain[2] = _LE_to_BE_long(((DCMD_TYPE_BMI | DCMD_BMI_OP_MOVE_I | DCMD_BMI_IO) 
    	    	<< 24) | count);
	    cmd_datain[3] = _LE_to_BE_long(buf);
#if 0
	...
	if (dataout) {
	    ...
	    cmd_dataout[0] = _LE_to_BE_long(((DCMD_TYPE_TCI | DCMD_TCI_OP_CALL) << 24) | 
		DBC_TCI_WAIT_FOR_VALID | DBC_TCI_COMPARE_PHASE);
	    cmd_dataout[1] = _LE_to_BE_long(virt_to_bus(hostdata->script) + 
    	    	hostdata->E_other_out);
	    /* MOVE count, buf, WHEN DATA+OUT */
	    cmd_dataout[2] = _LE_to_BE_long(((DCMD_TYPE_BMI | DCMD_BMI_OP_MOVE_I) << 24) 
		| count);
	    cmd_dataout[3] = _LE_to_BE_long(buf);
#if 0
    ...
    if (datain) {
	cmd_datain[0] = _LE_to_BE_long(((DCMD_TYPE_TCI | DCMD_TCI_OP_JUMP) << 24) |
    	    DBC_TCI_TRUE);
	cmd_datain[1] = _LE_to_BE_long(virt_to_bus(hostdata->script) + 
    	    hostdata->E_other_transfer);
#if 0
...
#endif
    if (dataout) {
	cmd_dataout[0] = _LE_to_BE_long(((DCMD_TYPE_TCI | DCMD_TCI_OP_JUMP) << 24) |
    	    DBC_TCI_TRUE);
	cmd_dataout[1] = _LE_to_BE_long(virt_to_bus(hostdata->script) + 
    	    hostdata->E_other_transfer);
#if 0
...
NCR53c7xx_queue_command (Scsi_Cmnd *cmd, void (* done)(Scsi_Cmnd *)) {
	...
	restore_flags (flags);
	cmd->result = _LE_to_BE_long(0xffff);	/* The NCR will overwrite message
...
to_schedule_list (struct Scsi_Host *host, struct NCR53c7x0_hostdata *hostdata,
	...
	cmd->dsa [(hostdata->dsa_jump_dest - hostdata->dsa_start) / 
	    sizeof(u32)] = (u32) _LE_to_BE_long(virt_to_bus ((void *)current));
	/* Replace the current jump operand.  */
	current[1] =
	    _LE_to_BE_long(virt_to_bus ((void *) cmd->dsa) + hostdata->E_dsa_code_begin -
	    hostdata->E_dsa_code_template);
	/* Replace the NOP instruction with a JUMP */
	current[0] = _LE_to_BE_long(((DCMD_TYPE_TCI|DCMD_TCI_OP_JUMP) << 24) |
	    DBC_TCI_TRUE);
    }  else {
	    ...
	    if (hostdata->issue_queue) {
			...
			if (!tmp->host_scribble || 
				...
				if (hostdata->options & OPTION_DEBUG_QUEUES) 
			    	...
			    	tmp->result = _LE_to_BE_long(tmp->result);
...
#ifdef __powerpc__
			if (tmp->result == _LE_to_BE_long(0xffff))
			    continue;
			tmp->result = _LE_to_BE_long(tmp->result);
#else			
			if (((tmp->result & 0xff) == 0xff) ||
...
intr_phase_mismatch (struct Scsi_Host *host, struct NCR53c7x0_cmd *cmd) {
	...
	if ((dcmd & (DCMD_TYPE_MASK|DCMD_BMI_OP_MASK|DCMD_BMI_INDIRECT|
		...
		cmd->residual[0] = _LE_to_BE_long(((DCMD_TYPE_TCI | DCMD_TCI_OP_CALL |
			((dcmd & DCMD_BMI_IO) ? DCMD_TCI_IO : 0)) << 24) | 
		    DBC_TCI_WAIT_FOR_VALID | DBC_TCI_COMPARE_PHASE);
		cmd->residual[1] = _LE_to_BE_long(virt_to_bus(hostdata->script)
		    + ((dcmd & DCMD_BMI_IO)
	     ...
	     */
	    cmd->residual[2] = _LE_to_BE_long(dbc_dcmd + residual);
	    cmd->residual[3] = _LE_to_BE_long(NCR53c7x0_read32(DNAD_REG) - residual);
	     ...
	     */
	    if (dsp != cmd->residual + 2) {
		cmd->residual[4] = _LE_to_BE_long(((DCMD_TYPE_TCI|DCMD_TCI_OP_JUMP) 
		    << 24) | DBC_TCI_TRUE);
		cmd->residual[5] = _LE_to_BE_long(virt_to_bus(dsp_next));
	    }
 ...
 */
	sprintf(buf, "%s0x%lx (virt 0x%p) : 0x%08x 0x%08x (virt 0x%p)", 
	    ...
	    insn[0], insn[1], bus_to_virt (_LE_to_BE_long(insn[1])));
	tmp = buf + strlen(buf);
	if ((dcmd & DCMD_TYPE_MASK) == DCMD_TYPE_MMI)  {
	    sprintf (tmp, " 0x%08x (virt 0x%p)\n", insn[2], 
		bus_to_virt(_LE_to_BE_long(insn[2])));
	    size = 3;
	} else {
	    sprintf (tmp, "\n");
	    size = 2;
 ...
 */ 
	...
	result = cmd->result;
#ifdef __powerpc__
	if (result == 0xffff0000) result = 0x0000ffff;
#endif
 ...
 */
	...
	((insn >= ncmd->data_transfer_start &&  
	    ...
	    ptr = bus_to_virt(_LE_to_BE_long(insn[3]));
...
print_progress (Scsi_Cmnd *cmd) {
	...
	if (!i) {
	    where = "saved";
	    ptr = bus_to_virt(_LE_to_BE_long(ncmd->saved_data_pointer));
	} else {
	    where = "active";
	...
	if (offset != -1) 
		...
		size = print_insn (cmd->host, 
		    bus_to_virt(_LE_to_BE_long(ncmd->saved_data_pointer)), "", 1);
		print_insn (cmd->host, 
		    bus_to_virt(_LE_to_BE_long(ncmd->saved_data_pointer)) + size * sizeof(u32),
		    "", 1);
	    }
	}
    }
...
print_dsa (struct Scsi_Host *host, u32 *dsa, const char *prefix) {
    ...
    printk("%sscsi%d : dsa at phys 0x%lx (virt 0x%p)\n"
	    "        + %d : dsa_msgout length = %u, data = 0x%x (virt 0x%p)\n" ,
    	    ...
    	    _LE_to_BE_long(dsa[hostdata->dsa_msgout / sizeof(u32)]),
	    _LE_to_BE_long(dsa[hostdata->dsa_msgout / sizeof(u32) + 1]),
	    bus_to_virt (_LE_to_BE_long(dsa[hostdata->dsa_msgout / sizeof(u32) + 1])));
    ...
    if (_LE_to_BE_long(dsa[hostdata->dsa_msgout / sizeof(u32)]) < 
	    sizeof (hostdata->free->select)) 
	for (i = _LE_to_BE_long(dsa[hostdata->dsa_msgout / sizeof(u32)]),
	    ptr = bus_to_virt (_LE_to_BE_long(dsa[hostdata->dsa_msgout / sizeof(u32) + 1])); 
	...
	hostdata->dsa_select, _LE_to_BE_long(dsa[hostdata->dsa_select / sizeof(u32)]));
    cmd = (Scsi_Cmnd *) bus_to_virt(_LE_to_BE_long(dsa[hostdata->dsa_cmnd / sizeof(u32)]));
	...
	_LE_to_BE_long(dsa[hostdata->dsa_next / sizeof(u32)]));
    if (cmd) { 
...
print_queues (struct Scsi_Host *host) {
	...
	if (cmd->host_scribble) {
	    ...
	    /* print_dsa does sanity check on address, no need to check */
	    	...
	    	print_dsa (host, _LE_to_BE_long(((struct NCR53c7x0_cmd *) cmd->host_scribble)-> dsa), "");
	} else 
...
/* FIXME : convert pointer to dsa_begin to pointer to dsa. */
	    print_dsa (host, bus_to_virt (_LE_to_BE_long(current[1]) - 
    ...
    for (left = host->can_queue, 
	dsa = bus_to_virt (_LE_to_BE_long(hostdata->reconnect_dsa_head));
	left >= 0 && dsa; 
	dsa = next_dsa) {
	save_flags (flags);
	cli();
	...
	{
	    next_dsa = bus_to_virt(_LE_to_BE_long(dsa[hostdata->dsa_next / sizeof(u32)]));
	    print_dsa (host, dsa, "");
	}
	restore_flags(flags);
    }
