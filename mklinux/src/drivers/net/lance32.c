...
#ifdef __powerpc__
#define _LE_l(x) _LE_to_BE_long(x)
#define _LE_s(x) _LE_to_BE_short(x)
#else
#define _LE_l(x) (x)
#define _LE_s(x) (x)
#endif
...
void lance32_probe1(struct device *dev, char *chipname, int pci_irq_line)
{
	...
	lp->init_block.mode = _LE_s(0x0003);		/* Disable Rx and Tx. */
        lp->init_block.tlen_rlen = _LE_s(TX_RING_LEN_BITS | RX_RING_LEN_BITS);    
	for (i = 0; i < 6; i++)
		lp->init_block.phys_addr[i] = dev->dev_addr[i];
	lp->init_block.filter[0] = 0x00000000;
	lp->init_block.filter[1] = 0x00000000;
	lp->init_block.rx_ring = (u32)_LE_l(virt_to_bus(lp->rx_ring));
	lp->init_block.tx_ring = (u32)_LE_l(virt_to_bus(lp->tx_ring));
...
lance32_init_ring(struct device *dev)
{
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		lp->rx_ring[i].base = (u32)_LE_l(virt_to_bus((char *)lp->rx_buffs + i*PKT_BUF_SZ));
		lp->rx_ring[i].buf_length = _LE_s(-PKT_BUF_SZ);
	        lp->rx_ring[i].status = _LE_s(0x8000);	    
	}
...
lance32_start_xmit(struct sk_buff *skb, struct device *dev)
{
	...
	lp->tx_ring[entry].length = _LE_s(-skb->len);

	lp->tx_ring[entry].misc = 0x00000000;

	lp->tx_skbuff[entry] = skb;
	lp->tx_ring[entry].base = (u32)_LE_l(virt_to_bus(skb->data));
        lp->tx_ring[entry].status = _LE_s(0x8300);
	...
	while ((csr0 = inw(dev->base_addr + LANCE_DATA)) & 0x8600
		...
		if (csr0 & 0x0200) {		/* Tx-done interrupt */
			...
			while (dirty_tx < lp->cur_tx) {
				...
				int status = (short)_LE_s(lp->tx_ring[entry].status);
				...
				if (status & 0x4000) {
					...
					int err_status = _LE_s(lp->tx_ring[entry].misc);
					lp->stats.tx_errors++;
...
lance32_rx(struct device *dev)
{
	...
	while ((short)_LE_s(lp->rx_ring[entry].status) >= 0) {
		int status = (short)_LE_s(lp->rx_ring[entry].status) >> 8;
		...
		if (status != 0x03) {			/* There was an error. */
			...
			lp->rx_ring[entry].status &= _LE_s(0x03ff);       /* Caution! */
		}
		else 
		{
			/* Malloc up new buffer, compatible with net-2e. */
			short pkt_len = (_LE_l(lp->rx_ring[entry].msg_length) & 0xfff)-4;
			struct sk_buff *skb;
			...
			else
			{
				...
				if (skb == NULL) 
				{
					...
					for (i=0; i < RX_RING_SIZE; i++)
						if ((short)_LE_s(lp->rx_ring[(entry+i) & RX_RING_MOD_MASK].status) < 0)
							break;

					if (i > RX_RING_SIZE -2) 
					{
						lp->stats.rx_dropped++;
						lp->rx_ring[entry].status |= _LE_s(0x8000);    /* Caution! */
						lp->cur_rx++;
					}
					break;
				}
				...
				eth_copy_and_sum(skb,
					(unsigned char *)bus_to_virt(_LE_l(lp->rx_ring[entry].base)),
					pkt_len,0);
		...
		lp->rx_ring[entry].buf_length = _LE_s(-PKT_BUF_SZ);
		lp->rx_ring[entry].status |= _LE_s(0x8000);    /* Caution! */
