...
/* CMU_HIST */
/*
 * Revision 2.5  92/05/22  15:48:34  jfriedl
 * 	Now all screens clearup at boot.
 * 	[92/05/21            af]
 * 
 * Revision 2.4  92/05/05  10:05:08  danner
 * 	Fixed setting of reset address (vdac_regs, not planemask!).
 * 	[92/04/14  11:54:42  af]
 * 
 * Revision 2.3  92/03/05  11:37:04  rpd
 * 	Cursor/VDAC chip works now.
 * 	[92/03/04            af]
 * 
 * Revision 2.2  92/03/02  18:33:13  rpd
 * 	Created.
 * 	[92/03/02            af]
 * 
 */
...
int	xcfb_probe(), xcfb_attach(), xcfb_intr();

caddr_t	xcfb_std[NXCFB] = { 0 };
	...
	xcfb_softc_t	*xcfb;
{
	...
	for (i = 0; i < NXCFB; i++)
		if (xcfb_info[i]->address == (caddr_t)xcfb->framebuffer)
			break;
	if (i == NXCFB) return;
...
/* some of these values are made up using ad hockery */
