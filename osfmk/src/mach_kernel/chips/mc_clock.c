 ...
 */
int mc_probe( vm_offset_t addr );
static int mc_attach();

int mc_intr();

void mc_open(), mc_close(), mc_write();
static unsigned int mc_read();

static void mc_wait_for_uip( mc_clock_ram_t *clock );
 ...
 */
static mc_clock_ram_t	*rt_clock = MC_DEFAULT_ADDRESS;

/*
 * (Auto?)Configuration
 */
static vm_offset_t mc_std[NMC] = { 0 };
