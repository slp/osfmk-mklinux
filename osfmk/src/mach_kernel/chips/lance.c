...
static vm_offset_t se_Hmem_nogap(), se_Hmem_gap16();
static vm_offset_t se_malloc();
 ...
 */
static vm_offset_t se_Hmem_nogap( vm_offset_t lna) { return lna;}
static vm_offset_t se_Hmem_gap16( vm_offset_t lna) { return lna << 1;}
...
static  void se_init();
static	void init_lance_space();
static  void se_desc_set_status();
static  volatile long *se_desc_alloc();	/* must be aligned! */
void	se_start();
	...
	void			*up)
{
	...
	struct bus_device *ui = (struct bus_device *)up;
	se_reg_t        rdp, rap;
	int             unit = ui->unit;
...
se_output(
	...
	io_req_t	ior)
{
    return net_write(&se_softc[dev]->is_if, (void(*)())se_start, ior);
}
