...
int scsi_play_audio_track_index(
	target_info_t	*tgt,
	int		st,
	int		si,
	int		et,
...
int scsi_play_audio_track_relative(
	target_info_t	*tgt,
	unsigned int	lba,
	int		st,
	unsigned int	len,
...
int scsi_play_audio_track_relative_long(
	target_info_t	*tgt,
	unsigned int	lba,
	int		st,
	unsigned int	len,
	...
	io_req_t	ior)
{
	...
	bzero((char *)tgt->cmd_ptr, sizeof(*cmd) + sizeof(*parm));
	...
	bcopy((char *)page, (char *)parm, pagesize);
...
int cdrom_vendor_specific(
	...
	io_req_t	ior)
{
	bcopy((char *)cmd, (char *)tgt->cmd_ptr, sizeof(*cmd));
	if (paramlen)
		bcopy((char *)params, tgt->cmd_ptr + sizeof(*cmd), paramlen);
